...
class EFT(PriorityHeuristic):
    """ Earliest Finish Time: Calculated by completing a CPM (critical
    path method) analysis. Tabulated by Lova, Tormos & Barber
    (2006) as one of the heuristics they considered."""
    @staticmethod
    def get_name() -> str:
        return "EFT"
    ...
    def prioritise(project: Project, mode_assignments: list[int]) -> list[int]:
        return CriticalPathMethodCalculator.get_cpm_schedule(project, mode_assignments).earliest_finish_time